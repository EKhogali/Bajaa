...
<div class="container row">
    <div class="container col-3">
        <form action="/estimated_expense_report?fromdate={{Request('fromdate')}}&todate={{Request('todate')}}" method="get">
            {{ csrf_field() }}
    ...
    <div class="container col-9" id="printableArea">
        ...
        <div class="row" style="border: 2px solid #6574cd;">
            <div class="col-12 text-center">
                ...
                <br>
                <h4 style="background-color: #f0f0f0; padding: 10px; border-radius: 5px;">تقرير: الدخل (التقديري)</h4>
                <br>
...
{{--                <tr style="{{ $backgroundColor }} {{ $lightBlueBold ? 'background-color: lightblue; font-weight: bold;' : '' }} {{ $totalRecord ? 'text-decoration: underline; font-style: italic;' : '' }}">--}}
                ...
                <th scope="row" width="4%">{{ $income_report->id ?? '' }}</th>
                    ...
                    <th scope="row" width="15%">{{ round( $number3 ?? 0 ,2)  }}</th>
                    <th scope="row" width="15%">{{ round($number4 ?? 0 ,2).'%' }}</th>
                    @else
